...
#if LOG_TO_RAMBUFFER
    static char TcLogRamBuffer[LOG_RAMBUFFER_SIZE] = {0};     /*!< 日志缓冲区大小 */
    static int TcLogRamBufferPos = 0;                   /*!< 日志缓冲区指针位置 */
#endif

static T_TcLogPrintPin printpin = PRINT_GPIO2;          //打印引脚
...
* @retval
*/
DEFINE_FAKE_VOID_FUNC(PrintInit, uint32_t);

DEFINE_FAKE_VOID_FUNC(PrintSetPin, T_TcLogPrintPin);

DEFINE_FAKE_VOID_FUNC(PrintEnterSleep);

DEFINE_FAKE_VOID_FUNC(PrintWakeup);
...
/**
  ...
  * @return
  */
DEFINE_FAKE_VOID_FUNC_VARARG(TcLogWrite, const char *, ...);
#endif
...
* @retval
*/
DEFINE_FAKE_VALUE_FUNC(int, TC_LOG_SYMBOL_NAMED, uint8_t, const char *, const void *, int);
...
* @retval
*/
DEFINE_FAKE_VALUE_FUNC(int, TC_LOG_SYSTEM_STATUS, T_TcLogSystemStatus);
...
* @retval
*/
DEFINE_FAKE_VALUE_FUNC(int, TC_LOG_RAWDATA, uint8_t, T_TcLogRawdataType, uint8_t, const void *, int);
...
* @retval       缓冲区数据长度
*/
DEFINE_FAKE_VALUE_FUNC(int, TC_LOG_GetRamBuffer, const char **);

/**
* @brief       清空缓冲区
* @retval
*/
DEFINE_FAKE_VOID_FUNC(TC_LOG_ClrRamBuffer);
#endif
