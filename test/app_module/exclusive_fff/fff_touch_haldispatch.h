...
#ifndef __FFF_TOUCH_HALDISPATCH_H__
#define __FFF_TOUCH_HALDISPATCH_H__

#include "fff.h"
// #include "fff_si_include.h"
#include "fff_si_hal.h"
...
#define TOUCH_SCAN_MANY 1                            // 分发器一次扫描多个通道，多个由scan_many_num设置
    ...
    uint16_t virtual_mergekey_num;                   // 虚拟合并按键个数，合并的按键值会相加
    const TOUCH_HalMergeKey_Type *virtual_mergekeys; // 虚拟合并按键配置
...
#define Touch_HalDispatch_SetInSleep(dispatch, sleep) \
    do                                                \
    {                                                 \
...
DECLARE_FAKE_VOID_FUNC(Touch_HalDispatch_SetFastScan, TOUCH_HalDispatch_Type *, uint8_t);
DECLARE_FAKE_VALUE_FUNC(TOUCH_HalDispatch_Type *, Touch_HalDispatchOnlyTouchCreate, TOUCH_HalDispatch_OnlyTouch_Type *, const TOUCH_HalScanerTable_Type *, const TOUCH_HalDispatch_OnlyTouchPara_Type *);
DECLARE_FAKE_VOID_FUNC(Touch_HalDispatch_DiscardSampData, int);
#endif
