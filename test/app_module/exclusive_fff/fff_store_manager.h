...
#ifndef __FFF_STORE_MANAGER_H__
#define __FFF_STORE_MANAGER_H__

#include "fff.h"
#include <stdint.h>
#include <stdbool.h>
#include "fff_store_map.h"
...
extern "C"
{
    ...
    DECLARE_FAKE_VOID_FUNC(store_manager_clear);
    DECLARE_FAKE_VOID_FUNC(store_manager_init);
    DECLARE_FAKE_VOID_FUNC(store_system_data_set, system_param_type_e, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_system_data_get, system_param_type_e, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_chip_info_get, chip_info_type_e, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_customer_data_set, uint32_t, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_customer_data_get, uint32_t, uint8_t *, uint16_t);
    DECLARE_FAKE_VOID_FUNC(store_default_whitepoint);
    DECLARE_FAKE_VALUE_FUNC(bool, store_slow_read, uint32_t, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_slow_smart_read, uint32_t, uint8_t *, uint16_t);
    DECLARE_FAKE_VALUE_FUNC(bool, store_slow_write, uint32_t, uint8_t *, uint16_t);
